...
{
    // Vérifiez si $content est nul avant d'accéder à ses propriétés
    ...
    if ($content) {
        ...
        // Gérer le cas où $content est nul (null)
        // Par exemple, définissez des valeurs par défaut ou affichez un message d'erreur
        // Ici, nous définissons des valeurs par défaut pour éviter l'erreur
        $this->seo()->setTitle("Titre par défaut");
        $this->seo()->setDescription("Description par défaut");
        SEOMeta::setKeywords("Mots-clés par défaut");
        $this->seo()->opengraph()->setUrl(url()->current());
        $this->seo()->opengraph()->addProperty('type', 'website');
        $this->seo()->twitter()->setSite(url()->current());
        $this->seo()->jsonLd()->setType('Website');
